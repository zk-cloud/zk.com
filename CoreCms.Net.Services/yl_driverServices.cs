...
namespace CoreCms.Net.Services
{
    ...
    ///  接口实现
    /// </summary>
    public class yl_driverServices : BaseServices<yl_driver>, Iyl_driverServices
    {
        private readonly Iyl_driverRepository _dal;
        private readonly IUnitOfWork _unitOfWork;
        private readonly ICoreCmsSmsServices _smsServices;
        private readonly ICoreCmsUserWeChatInfoServices _userWeChatInfoServices;
        private readonly PermissionRequirement _permissionRequirement;

        public yl_driverServices(IUnitOfWork unitOfWork
            , Iyl_driverRepository dal
        ...
        public async Task<WebApiCallBack> SmsLogin(FMWxAccountCreate entity, int loginType = (int)GlobalEnumVars.LoginType.WeChatPhoneNumber, int platform = 1)
        {
            ...
            if (!string.IsNullOrEmpty(entity.sessionAuthId))
            {
                ...
                if (wxUserInfo != null)
                {
                    await _dal.UpdateAsync(p => new yl_driver() { name = wxUserInfo.nickName }, p => p.id == userInfo.id);
                }
            }
